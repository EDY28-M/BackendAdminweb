...
export class RidersService {
  ...
  private async getActiveRiderByUserId(userId: string) {
    ...
    if (!rider) {
      throw new ForbiddenException('No tienes perfil de repartidor');
    }
    if (rider.status !== 'active') {
      throw new ForbiddenException('Tu cuenta de repartidor no está activa');
    }
    return rider;
  }
  ...
  async acceptOrder(userId: string, orderId: string) {
    ...
    // El repartidor puede aceptar el pedido en cualquier momento desde que se crea
    // Se eliminó la restricción de que el pedido debe estar "ready_for_pickup"

    if (
      order.deliveries.rider_profile_id &&
  ...
  async rejectOrder(userId: string, orderId: string, reason?: string) {
    ...
    if (order.status !== 'ready_for_pickup') {
      throw new BadRequestException('El pedido ya no está disponible para rechazo');
    }
  ...
  async updateMyOrderStatus(
    ...
    if (order.deliveries.rider_profile_id !== rider.id) {
      throw new ForbiddenException('Este pedido no está asignado a tu cuenta');
    }
    ...
    if (status === 'picked_up') {
      ...
      if (!order.picked_up_at) orderUpdate.picked_up_at = now;
      if (!order.deliveries.picked_up_at) deliveryUpdate.picked_up_at = now;
